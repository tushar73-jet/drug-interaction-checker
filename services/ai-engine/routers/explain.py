...
async def explain_interaction(body: ExplainRequest) -> Any:
    """
    3-stage AI pipeline:
    1. Groq LLM decomposes the clinical question into targeted search queries.
    2. Tavily searches medical literature for each query in parallel.
    3. Groq LLM synthesizes a clinical narrative with inline source citations.
    """
    ...
    try:
        ...
        logger.error("Explain pipeline timed out for %s + %s", body.drug1, body.drug2)
        raise HTTPException(
            status_code=504,
            detail="AI explanation timed out. Please try again.",
        )
    except Exception as exc:
        logger.exception("Explain pipeline failed for %s + %s: %s", body.drug1, body.drug2, exc)
        raise HTTPException(
            status_code=502,
